...
import type { Attachment, ToolMode } from "@/lib/types";
...
export function BottomComposer({
  ...
  const isSendDisabled = (!isGenerating && !value.trim()) || isQuotaExceeded;
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={clsx(
                    "inline-flex h-11 w-11 items-center justify-center rounded-full transition",
                    isGenerating
                      ? "bg-text-primary text-white shadow-soft hover:bg-black/80"
                      : !isSendDisabled
                        ? "bg-primary text-white shadow-soft hover:bg-primary-hover"
                        : "cursor-not-allowed bg-surface-highest text-text-muted"
                  )}
                >
                  {isGenerating ? <Square className="h-4 w-4" /> : <SendHorizonal className="h-4 w-4" />}
                </button>
              </div>
            </div>
          </div>
