...
import { EmptyHome } from "@/features/chat/components/empty-home";
import type { Capability, ChatSummary, Message, Suggestion } from "@/shared/types";

const MessageList = dynamic(
  async () => {
    const module = await import("@/features/chat/components/message-list");
...
export function ChatStage({
  ...
  const chatTitle =
    activeChat?.title ??
    (messages.find((message) => message.role === "user")?.content.slice(0, 18) || "新的对话");
              ...
              <div className="mx-auto mb-6 flex w-full max-w-chat items-center justify-between rounded-xl bg-card/50 px-5 py-4 backdrop-blur-sm">
                <div>
                  ...
                  <p className="mt-1 text-sm text-text-secondary">
                    {activeModel ?? "No model selected"} · Ready for rename, share, and future actions.
                  </p>
                </div>
                <div className="hidden items-center gap-2 md:flex">
                  ...
                  <span className="rounded-full bg-card px-3 py-1 text-xs font-medium text-text-secondary">
                    {activeStreamCount > 0 ? `Streaming ${activeStreamCount}` : "Idle"}
                  </span>
                </div>
              </div>

              <div className="relative flex min-h-0 flex-1 overflow-hidden">
                <MessageList messages={messages} bottomOffset={messageViewportGap} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
